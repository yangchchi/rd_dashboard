...
const AI_SKILL_STORAGE_KEY = '__global_rd_ai_skill_configs';
...
const DEFAULT_AI_SKILLS: Record<string, IAiSkillConfig> = {
  [PRD_GENERATION_SKILL_ID]: {
...
function readSkillConfigs(): Record<string, IAiSkillConfig> {
  try {
    const raw = localStorage.getItem(AI_SKILL_STORAGE_KEY);
    if (!raw) return { ...DEFAULT_AI_SKILLS };
    const parsed = JSON.parse(raw) as Record<string, IAiSkillConfig>;
    return { ...DEFAULT_AI_SKILLS, ...parsed };
  } catch {
    return { ...DEFAULT_AI_SKILLS };
  }
}

function writeSkillConfigs(configs: Record<string, IAiSkillConfig>) {
  localStorage.setItem(AI_SKILL_STORAGE_KEY, JSON.stringify(configs));
}
...
export function listAiSkills(): IAiSkillConfig[] {
  return sortSkills(Object.values(readSkillConfigs()));
}

export function getAiSkill(skillId: string): IAiSkillConfig {
  const skill = readSkillConfigs()[skillId];
  if (!skill) {
...
export function updateAiSkill(skillId: string, patch: Partial<IAiSkillConfig>) {
  const current = readSkillConfigs();
  const target = current[skillId];
  if (!target) {
    throw new Error(`未找到Skill: ${skillId}`);
  }
  const next = {
  ...
  writeSkillConfigs(next);
}

export function registerAiSkill(skill: IAiSkillConfig) {
  const current = readSkillConfigs();
  if (current[skill.id]) {
    throw new Error(`Skill 已存在: ${skill.id}`);
  }
  const next = {
    ...current,
    [skill.id]: skill,
...
export function resetAiSkill(skillId: string) {
  const def = DEFAULT_AI_SKILLS[skillId];
  if (!def) {
    throw new Error(`无内置默认配置: ${skillId}`);
  }
  const current = readSkillConfigs();
  const next = { ...current, [skillId]: { ...def } };
  writeSkillConfigs(next);
}

export function resetAiSkills() {
  writeSkillConfigs({ ...DEFAULT_AI_SKILLS });
}
